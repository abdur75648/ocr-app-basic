...
def prepare_for_boundary_segmentation(predictions, dataset):
    ...
    masker = Masker(threshold=0.6, padding=1)
    coco_results = []
    ...
    for image_id, prediction in tqdm(enumerate(predictions)):
        ...
        if 'ic15' in cfg.DATASETS.TEST[0]:
            ...
            # rects = merge_rectangles(rects,30)
            
            # print("rects:\t"+str(rects)) # [array([ 598, 1013,  464, 1013,  464,  933,  598,  933]), array([ 257, 1544,  257, 1392,  754, 1392,  754, 1544]),...]
        if 'CTW' in cfg.DATASETS.TEST[0]:
            contours = [mask_to_contours(mask, [image_height, image_width]) for mask in masks]
            # output for evaluation
            write_result_as_txt(image_name, contours, 'ctw_eval/results')
            # visualization
            if cfg.DATASETS.Test_Visual:
                im_write = cv2.imread(
                    'datasets/CTW_dataset/test_img/' + im_w_name)[:, :,::-1]
                for box in contours:
                    box = np.array(box)
                ...
                if not os.path.exists("vis"):
                    os.mkdir("vis")
                if not os.path.exists("ctw"):
                    os.mkdir("ctw")
                cv2.imwrite('vis/ctw/' + im_w_name,im_write[:, :, ::-1])
