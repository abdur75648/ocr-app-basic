...
class Encoder (nn.Module):
    ...
    def forward (self , x):
        ...
        # print ("Final CNN Output", x.shape)
        # (512, 4, 101)
...
class Decoder (nn.Module):
    ...
    def __init__(self, block, layers, chs = (256, 128, 64),):
        ...
        self.inplanes = int(512)
        
        # #! Fix this shape. (add x to out_channels = 256 )
        # self.convt1 = nn.ConvTranspose2d(128, 128, 2, 2)
        # self.layer1 = self._make_layer(block, 64, 1, stride=1, in_ch_=128)
        # self.conv1 = nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1, bias=False)
        # self.bn1 = nn.BatchNorm2d(128)        
        
        
        # #! Fix this shape. (add x to out_channels = 256 )
        # self.convt2 = nn.ConvTranspose2d(256, 256, 2, 2)
        # self.layer2 = self._make_layer(block, 128, 2, stride=1, in_ch_=256)
        # self.conv2 = nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1, bias=False)
        # self.bn2 = nn.BatchNorm2d(256)
        
        # self.convt3 = nn.ConvTranspose2d(512, 512, 2, (2,1), (0,1))
        # self.layer3 = self._make_layer(block, 256, 5, stride=1, in_ch_=512)
        # self.conv3 = nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=1, bias=False)
        # self.bn3 = nn.BatchNorm2d(512)

        #! Fix this shape. (add x to out_channels = 256 )
                
        self.convt1 = nn.ConvTranspose2d(512, 448, 2, 2)
        self.conv1 = nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(512)
        self.layer1 = self._make_layer(block, 512, 1, stride=1, in_ch_=512)        
        
        #! Fix this shape. (add x to out_channels = 256 )
        self.convt2 = nn.ConvTranspose2d(512, 384, 2, 2)
        self.conv2 = nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(512)
        self.layer2 = self._make_layer(block, 512, 2, stride=1, in_ch_=512)
        
        self.convt3 = nn.ConvTranspose2d(512, 256, 2, (2,1), (0,1))
        self.conv3 = nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn3 = nn.BatchNorm2d(512)
        self.layer3 = self._make_layer(block, 512, 5, stride=1, in_ch_=512)
        
        self.convt4 =  nn.ConvTranspose2d(512, 256, 3, 1, 1)
        self.conv4 = nn.Conv2d(256, 512, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn4 = nn.BatchNorm2d(512)
        self.layer4 = self._make_layer(block, 512, 5, stride=1, in_ch_=512)
        

    def _make_layer(self, block, planes, blocks, stride=1, in_ch_ = 256):
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:
            # print ("YEYSYYSYSY")
            downsample = nn.Sequential(
                nn.Conv2d(in_ch_, planes * block.expansion,
                          kernel_size=1, stride=stride, bias=False),
                nn.BatchNorm2d(planes * block.expansion),
            )

        layers = []
        layers.append(block(in_ch_, planes, stride, downsample))
        in_ch_ = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(in_ch_, planes))

        return nn.Sequential(*layers) 
        
    def forward (self, x, encoder_features = None):
        
        # for kk in encoder_features:
        #     print("kk", kk.shape)
...
class UNet_FeatureExtractor(nn.Module):
    ...
    def __init__(self, input_channel, output_channel = 512):
        ...
        self.ConvNet = UNet(input_channel, output_channel, BasicBlock, [1, 2, 5, 3])
...
class UNet (nn.Module):
    ...
    def __init__(self, input_channel, output_channel, block, layers):
        ...
        self.encoder = Encoder(input_channel, output_channel, block, layers)
        self.decoder = Decoder(block, layers)
    ...
    def forward (self, x):
        ...
        # visual_feature = self.AdaptiveAvgPool(out.permute(0, 3, 1, 2))  # [b, c, h, w] -> [b, w, c, h]
...
# net = UNet_FeatureExtractor(1,512)
# out = net(x)
# print (out.shape)